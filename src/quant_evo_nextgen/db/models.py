...
MONEY_NUMERIC = Numeric(24, 8, asdecimal=False)
PRICE_NUMERIC = Numeric(20, 8, asdecimal=False)
QUANTITY_NUMERIC = Numeric(24, 10, asdecimal=False)
RATIO_NUMERIC = Numeric(18, 8, asdecimal=False)
...
class BrokerAccountSnapshotModel(Base, LineageMixin, TimestampMixin):
    ...
    environment: Mapped[str] = mapped_column(String(40), nullable=False, index=True)
    equity: Mapped[float] = mapped_column(MONEY_NUMERIC, default=0.0, nullable=False)
    cash: Mapped[float] = mapped_column(MONEY_NUMERIC, default=0.0, nullable=False)
    buying_power: Mapped[float] = mapped_column(MONEY_NUMERIC, default=0.0, nullable=False)
    gross_exposure: Mapped[float] = mapped_column(MONEY_NUMERIC, default=0.0, nullable=False)
    net_exposure: Mapped[float] = mapped_column(MONEY_NUMERIC, default=0.0, nullable=False)
...
class InstrumentDefinitionModel(Base, LineageMixin, TimestampMixin):
    ...
    strike_price: Mapped[float | None] = mapped_column(PRICE_NUMERIC)
    contract_multiplier: Mapped[float] = mapped_column(RATIO_NUMERIC, default=1.0, nullable=False)
    leverage_ratio: Mapped[float] = mapped_column(RATIO_NUMERIC, default=1.0, nullable=False)
...
class OrderIntentModel(Base, LineageMixin, TimestampMixin):
    ...
    quantity: Mapped[float] = mapped_column(QUANTITY_NUMERIC, nullable=False)
    reference_price: Mapped[float] = mapped_column(PRICE_NUMERIC, nullable=False)
    requested_notional: Mapped[float] = mapped_column(MONEY_NUMERIC, nullable=False)
    limit_price: Mapped[float | None] = mapped_column(PRICE_NUMERIC)
    stop_price: Mapped[float | None] = mapped_column(PRICE_NUMERIC)
    requested_at: Mapped[Any] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)
...
class OrderRecordModel(Base, LineageMixin, TimestampMixin):
    ...
    time_in_force: Mapped[str] = mapped_column(String(20), default="day", nullable=False)
    quantity: Mapped[float] = mapped_column(QUANTITY_NUMERIC, nullable=False)
    filled_quantity: Mapped[float] = mapped_column(QUANTITY_NUMERIC, default=0.0, nullable=False)
    requested_notional: Mapped[float] = mapped_column(MONEY_NUMERIC, default=0.0, nullable=False)
    avg_fill_price: Mapped[float | None] = mapped_column(PRICE_NUMERIC)
    limit_price: Mapped[float | None] = mapped_column(PRICE_NUMERIC)
    stop_price: Mapped[float | None] = mapped_column(PRICE_NUMERIC)
...
class OrderLegModel(Base, LineageMixin, TimestampMixin):
    ...
    position_effect: Mapped[str] = mapped_column(String(20), default="auto", nullable=False)
    quantity: Mapped[float] = mapped_column(QUANTITY_NUMERIC, nullable=False)
    ratio_quantity: Mapped[float] = mapped_column(RATIO_NUMERIC, default=1.0, nullable=False)
    reference_price: Mapped[float] = mapped_column(PRICE_NUMERIC, nullable=False)
    requested_notional: Mapped[float] = mapped_column(MONEY_NUMERIC, default=0.0, nullable=False)
...
class PositionRecordModel(Base, LineageMixin, TimestampMixin):
    ...
    market_price: Mapped[float | None] = mapped_column(PRICE_NUMERIC)
    notional_value: Mapped[float] = mapped_column(MONEY_NUMERIC, default=0.0, nullable=False)
    realized_pnl: Mapped[float] = mapped_column(MONEY_NUMERIC, default=0.0, nullable=False)
    unrealized_pnl: Mapped[float] = mapped_column(MONEY_NUMERIC, default=0.0, nullable=False)
...
class OptionLifecycleEventModel(Base, LineageMixin, TimestampMixin):
    ...
    instrument_key: Mapped[str | None] = mapped_column(String(200), index=True)
    quantity: Mapped[float] = mapped_column(QUANTITY_NUMERIC, nullable=False)
    event_price: Mapped[float | None] = mapped_column(PRICE_NUMERIC)
    cash_flow: Mapped[float | None] = mapped_column(MONEY_NUMERIC)
