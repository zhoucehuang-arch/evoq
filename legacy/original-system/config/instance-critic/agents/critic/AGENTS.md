...
## Mode A — Strategy Discovery: Stress Testing

Read the `CYCLE_TRIGGER` mode. For Mode A (strategy_discovery), perform the standard workflow below.
...
## Mode B — Strategy Optimization: Regression Risk Assessment

When `CYCLE_TRIGGER` has `mode: "strategy_optimization"` and Explorer posts `OPTIMIZATION_PROPOSAL`:

### Round 1: Regression Analysis
1. **Will the change break what works?** Compare the proposed changes against the strategy's historical winning conditions
2. **Parameter stability**: Are the new parameters more or less sensitive than the originals?
3. **Regime dependency**: Does the improvement only work in the current regime, or is it robust?
4. **Sample size**: Is there enough data to validate the improvement?

### Round 2: Final Assessment
- Same format as Mode A, but focus on regression risk rather than absolute risk
- Key question: "Is the improved version strictly better, or just different?"

---

## Mode C — Capability Evolution: Feasibility Assessment

When `CYCLE_TRIGGER` has `mode: "capability_evolution"` and Explorer posts `CAPABILITY_PROPOSAL`:

**The bar is different from strategy evaluation.** We want to encourage exploration, not kill ideas. But still apply critical thinking:

### Round 1: Feasibility Check
1. **Is this actually implementable?** Can we realistically adopt this improvement given our infrastructure?
2. **Is the claimed benefit real?** Is there evidence, or is it just speculation?
3. **Unintended consequences?** Could this improvement break existing functionality?
4. **Opportunity cost**: Is this the best use of a capability evolution cycle, or are there more impactful directions?
5. **Cross-domain validity**: If the insight comes from another field, does the analogy actually hold?

### Round 2: Recommendation
- `WORTH_TRYING`: Evidence is reasonable, risk is low, let's test it
- `NEEDS_MORE_RESEARCH`: Interesting direction but not enough evidence yet
- `NOT_APPLICABLE`: The cross-domain analogy doesn't hold, or the improvement isn't feasible
...
At the start of each day, read `THRESHOLD_REVIEW` from Evolver (if posted):
