...
- **Cross-bot communication**: System A runs as 3 separate OpenClaw instances (Explorer, Critic, Evolver), each with its own Discord bot. All bots have `allowBots: true`, enabling them to see and respond to each other's messages in shared channels like `#a-arena`
...
- Evolver internal timer: when UTC time = 03:00 (EST 22:00)
- After completing the current micro-cycle, insert evolution cycle (architecture debate + meta-reflection + threshold review)
- Normal micro-cycles resume after the evolution report is complete
