# 4-Instance OpenClaw · Self-Evolution + Automated Investment · System Architecture Design

## 0. Design Constraints Review

| Constraint | Design Response |
|---|---|
| 4-instance isolation (3 for System A, 1 for System B) | Explorer / Critic / Evolver each run as independent OpenClaw instances with own bot token; B is a single instance with 3 agents |
| Intra-instance Agent isolation | Each Agent has independent API Key, independent workspace, independent session |
| Minimalist and flat | A = 3 instances × 1 agent, B = 1 instance × 3 agents, 6 agents total |
...
| Cross-bot debate via allowBots: true | System A agents run under separate bot tokens; Discord channels set `allowBots: true` so bots can read/react to each other's messages, enabling the triangular game across instances |
...
│                           Discord Server                                  │
│                                                                           │
│  ┌─ instance-explorer ─┐  ┌─ instance-critic ─┐  ┌─ instance-evolver ─┐  │
│  │  Bot Token Explorer  │  │  Bot Token Critic  │  │  Bot Token Evolver │  │
│  │                      │  │                    │  │                    │  │
│  │  Explorer ◄──────────┼──┼── Critic ──────────┼──┼──► Evolver        │  │
│  │  (Propose)           │  │  (Challenge)       │  │  (Adjudicate)     │  │
│  └──────────┬───────────┘  └────────┬───────────┘  └────────┬──────────┘  │
│             │     Triangular Game (cross-bot via DC)        │             │
│             │                                               │             │
│  ┌─── instance-b (Investment) ──────────────────────────────┼──────────┐  │
│  │  Bot Token B                                             │          │  │
│  │                                                          │          │  │
│  │  Operator ◄──► Trader ◄──► Guardian                      │          │  │
│  └──────────────────────┬───────────────────────────────────┘          │  │
│                         │                                              │  │
│             ┌───────────▼──────────────┐                               │  │
│             │    GitHub Repository     │◄──────────────────────────────┘  │
...
**The four OpenClaw instances run completely independently, exchanging data only through the GitHub repository.**
**System A's three instances communicate via Discord channels (allowBots: true), enabling cross-bot triangular debate.**
...
### OpenClaw A: Self-Evolution System (3 Instances × 1 Agent)
...
- System A channels use `allowBots: true` so the three separate bot tokens can read and respond to each other's messages
- Minimize channel count: A system 4, B system 4, shared 2 = 10 total

### Channel List

**OpenClaw A Channels (Bot Token Explorer / Critic / Evolver — 3 separate bots, cross-bot via allowBots: true):**

| Channel | Writing Bot(s) | Reading Bot(s) | Purpose | Admin Can |
|---|---|---|---|---|
| `#a-arena` | Explorer, Critic | Explorer, Critic, Evolver | Debate arena: hypothesis proposal, challenges, responses | Observe debates, insert opinions |
| `#a-verdict` | Evolver | Explorer, Critic | Adjudication results + backtest reports + strategy push notifications | Approve/reject strategy pushes |
| `#a-research` | Explorer | Critic, Evolver | Paper/news discoveries, study notes | Specify research directions |
| `#a-report` | Evolver | Explorer, Critic | 2h/daily/weekly reports | View progress, adjust priorities |

> Note: Each System A channel must have `allowBots: true` in Discord channel permissions so that messages posted by one bot token are visible to the other bot tokens. Without this, the triangular game cannot function across instances.

**OpenClaw B Channels (Bot Token B — single bot, 3 agents):**
...
**Shared Channels (All Bots can write):**
...
│
├── config/
│   ├── instance-explorer/ # Explorer instance config
│   │   ├── openclaw.json
│   │   ├── .env.template
│   │   └── agents/explorer/
│   ├── instance-critic/   # Critic instance config
│   │   ├── openclaw.json
│   │   ├── .env.template
│   │   └── agents/critic/
│   ├── instance-evolver/  # Evolver instance config
│   │   ├── openclaw.json
│   │   ├── .env.template
│   │   └── agents/evolver/
│   ├── instance-b/        # B instance config (3 agents)
│   │   ├── openclaw.json
│   │   ├── .env.template
│   │   └── agents/
│   │       ├── operator/
│   │       ├── trader/
│   │       └── guardian/
│   └── risk-params.json   # Risk parameters (shared by all systems)
│
└── .github/
    └── workflows/
...
- All System A instances share `GITHUB_TOKEN_A` (same read/write scope: `strategies/candidates/`, `evo/`, `memory/`, `knowledge/`; read-only `trading/metrics/`)
- B instance uses `GITHUB_TOKEN_B` (read/write `strategies/staging/`, `strategies/production/`, `trading/`, `config/risk-params.json`; read-only `strategies/candidates/`)
...
### 5.2 Synthesis Cycle (Every 6 Hours)

```
Evolver aggregates recent micro-cycle results (~3 rounds per synthesis)
    │
    ▼
Identify recurring patterns (>=2 similar failures = pattern)
    │
    ▼
Synthesize into principles → write to memory/principles/
    │
    ▼
Cross-strategy analysis: compare approved strategies for common factors
    │
    ▼
Post synthesis summary to #a-report
    │
    ▼
commit to evo/cycles/synthesis-YYYY-MM-DD-HH.json
```

### 5.3 Evolution Cycle (Daily at 22:00 EST)

```
Evolver aggregates all micro-cycle and synthesis results for the day
    │
    ▼
Architecture-level debate: Explorer proposes system improvements,
Critic challenges, Evolver adjudicates
    │
    ▼
Meta-reflection: analyze debate quality, approve rates, mode distribution
    │
    ▼
Critic threshold adaptation: review false-positive/negative rates
    │
    ▼
Generate evolution report to #a-report:
  - Today's evolution results: N new strategies, M optimizations
  - Feature Map coverage changes
  - Capability evolution outcomes
  - Live trading performance feedback summary
  - Priority directions for tomorrow
    │
    ▼
commit to evo/cycles/evolution-YYYY-MM-DD.json + memory/architecture/
```
...
Each instance only needs 1 primary API key since each System A instance runs a single agent.

| Agent | Instance | Default Model | API Key | Core Usage |
|---|---|---|---|---|
| Explorer | instance-explorer | GPT-5.3 Codex | `OPENAI_KEY_A1` | Research + strategy generation (high creativity) |
| Critic | instance-critic | GPT-5.3 Codex | `OPENAI_KEY_A2` | Stress testing + risk analysis (high reasoning) |
| Evolver | instance-evolver | GPT-5.3 Codex | `OPENAI_KEY_A3` | Adjudication + backtesting + reporting (high synthesis) |
| Operator | instance-b | GPT-5.3 Codex | `OPENAI_KEY_B1` | Strategy deployment + task scheduling + reporting (high coordination) |
| Trader | instance-b | GPT-5.3 Codex | `OPENAI_KEY_B2` | Signal generation + execution (high precision) |
| Guardian | instance-b | GPT-5.3 Codex | `OPENAI_KEY_B3` | Risk monitoring + performance collection (high vigilance) |
...
└── deploy/                      # All deployment configurations
    ├── instance-explorer/       # OpenClaw Explorer (Self-Evolution: Propose)
    │   ├── openclaw.json        # Explorer instance main config
    │   ├── .env.template        # Explorer instance environment variable template
    │   └── agents/
    │       └── explorer/
    │           └── SOUL.md
    │
    ├── instance-critic/         # OpenClaw Critic (Self-Evolution: Challenge)
    │   ├── openclaw.json        # Critic instance main config
    │   ├── .env.template        # Critic instance environment variable template
    │   └── agents/
    │       └── critic/
    │           └── SOUL.md
    │
    ├── instance-evolver/        # OpenClaw Evolver (Self-Evolution: Adjudicate)
    │   ├── openclaw.json        # Evolver instance main config
    │   ├── .env.template        # Evolver instance environment variable template
    │   └── agents/
    │       └── evolver/
    │           └── SOUL.md
    │
