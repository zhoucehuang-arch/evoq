"""Adaptive Basket Variant: Momentum Breakout v1"""

STRATEGY_META = {
    "id": "adaptive_basket_momo_breakout_v1",
    "name": "Adaptive Basket Momentum Breakout v1",
    "version": "1.0.0",
    "archetype": "momentum",
    "asset_class": "equity",
...
def generate_signal(bars, params=None):
    ...
    if trend_ok and breakout and rsi >= params["rsi_entry_min"] and vol_mult >= params["volume_spike_mult"]:
        ...
        confidence = min(0.9, 0.50 + 0.12 * min(vol_mult, 2.5) + 0.10 * min((rsi - 50) / 20, 1.0) + 8.0 * breakout_strength)
        return {"action": "BUY", "confidence": round(confidence, 2), "reason": f"momo_entry rsi={rsi:.1f} vol={vol_mult:.2f} brk={price/prev_high:.3f}"}

    if rsi >= params["rsi_exit_min"] or price < fast or fast < slow:
        return {"action": "SELL", "confidence": 0.7, "reason": f"momo_exit rsi={rsi:.1f} fast_slow={fast/slow:.3f}"}
