"""
Solid Strategy v2: Alternative-Flow + Technical Confluence (Event-Aware Mutation)

Mutation goals:
- Higher entry quality via stricter flow/RSI thresholds.
- Faster risk-off exits under weak trend.
- Keep short holding profile for fast adaptation cycles.
"""

STRATEGY_META = {
    "id": "solid_altflow_technical_v2",
    "name": "Alternative-Flow + Technical Confluence v2",
    "version": "2.0.0",
    "archetype": "multi_factor",
    "asset_class": "equity",
    "holding_period_minutes": [60, 4320],
    "symbols": ["AAPL", "MSFT", "NVDA", "AMZN", "META", "GOOGL"],
    "signal_sources": ["options_flow_proxy", "technical", "event_risk_overlay"],
    "params": {
        "rsi_period": 14,
        "rsi_entry_max": 38,
        "rsi_exit_min": 57,
...
def generate_signal(bars, params=None):
    ...
    if abnormal_flow_proxy and rsi <= params["rsi_entry_max"] and price >= sma:
        flow_strength = min(1.0, volume / max(volume_avg, 1))
        confidence = min(0.88, 0.58 + 0.12 * flow_strength)
        return {
            "action": "BUY",
            "confidence": round(confidence, 2),
            "reason": (
                f"v2 entry: RSI={rsi:.1f}, vol_spike={volume/volume_avg:.2f}x, "
                f"price_vs_sma={price/sma:.3f}"
            ),
        }

    if rsi >= params["rsi_exit_min"] or price < sma * 0.992:
        return {
            "action": "SELL",
            "confidence": 0.72,
            "reason": f"v2 exit: RSI={rsi:.1f}, price_vs_sma={price/sma:.3f}",
        }
